...
Public Class Admin_TimeLogs
    ...
    Sub loadrecords()
        Try
            ...
            conn.Open()
            comm = New SqlCommand("Select * From AdminLogs where Username ='" & Loginfrm.BunifuMaterialTextbox1.Text & "'", conn)
    ...
    Private Sub BunifuDatepicker1_onValueChanged(sender As Object, e As EventArgs) Handles BunifuDatepicker1.onValueChanged
        ...
        If enter = True Then
            ...
            Try
                conn = New SqlConnection(constr)
                conn.Open()
                comm = New SqlCommand("SELECT ID, Username, Login, Duration,Logout FROM [CGL.RME].[dbo].[AdminLogs] Where Username='" & Loginfrm.BunifuMaterialTextbox1.Text & "' and Convert(VARCHAR(10),Login,101) =  Convert(Varchar(10),'" & BunifuDatepicker1.Value.ToString("MM/dd/yyyy") & "',101)", conn)
                reader = comm.ExecuteReader
                ListView1.Items.Clear()
                Do While reader.Read = True
                    lst = ListView1.Items.Add(reader(1).ToString)
